...
data = [
   ...
   },
   {
       ...
       "summary": "The Cold War is a relatively new history channel on youtube which focuses on world events that occurred during the Cold War. It covers major topics like the Korean War and the Suez Crisis and also lesser known events like the Poznan uprising, the Kuril islands dispute and the Algerian War of Independence.",
...
def search(searchkey):
    ...
    res = []
    for d in data:
        genres_string = ", ".join(d["genres"])
        if searchkey.lower() in d["title"].lower():
            item_result = d
            item_result["matched"] = "title"
            item_result["start"] = d["title"].lower().find(searchkey.lower())
            item_result["end"] = item_result["start"] + len(searchkey)
            res.append(item_result)
        elif searchkey.lower() in d["summary"].lower():
            item_result = d
            item_result["matched"] = "summary"
            pos = d["summary"].lower().find(searchkey.lower())
            i = pos - d["summary"].lower()[:pos][::-1].find(".")
            j = pos + d["summary"].lower()[pos:].find(".")
            item_result["extra"] = d["summary"][i+1:j+1]
            item_result["start"] = item_result["extra"].lower().find(searchkey.lower())
            item_result["end"] = item_result["start"] + len(searchkey)
            res.append(item_result)
        elif searchkey.lower() in genres_string.lower():
            item_result = d
            item_result["matched"] = "genres"
            item_result["start"] = genres_string.lower().find(searchkey.lower())
            item_result["end"] = item_result["start"] + len(searchkey)
            item_result["extra"] = genres_string
            res.append(item_result)
...
def add_item():
    ...
    s = new_item["genres"]
    new_item["genres"] = [x.strip(" ") for x in s.split(",")]
...
def edit_item():
    ...
    s = item_updated["genres"]
    item_updated["genres"] = [x.strip(" ") for x in s.split(",")]
